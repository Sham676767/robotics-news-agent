...
def generate_article(top5: list[dict[str, Any]], api_key: str | None = None) -> dict[str, Any]:
    ...
    prompt = (
        ...
        "Каждую цифру, дату, сумму или размер используй только если это значение буквально есть в соответствующей карточке; если сомневаешься, опиши факт без числа. "
    ...
    for repair_attempt in range(MAX_ARTICLE_REPAIR_ATTEMPTS + 1):
        try:
            ...
            repair_prompt = (
                ...
                "Числа: оставляй цифру, дату, сумму или размер только если это значение буквально есть в соответствующей карточке; иначе убери числовое утверждение целиком, не заменяя его догадкой. "
