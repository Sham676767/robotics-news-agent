...
6. GigaChat API со строгим JSON-выводом для отбора TOP-5 и редакторской обработки.
...
Скопировать `.env.example` в `.env` и заполнить только необходимые переменные. Для GigaChat нужен долгоживущий ключ авторизации в `GIGACHAT_AUTHORIZATION_KEY`; краткоживущие токены доступа программа получает сама. Файл `.env` не должен попадать в Git.
