...
Скопировать `.env.example` в `.env` и заполнить только необходимые переменные. Для OpenRouter нужен ключ в `OPENROUTER_API_KEY`; закреплённая модель задаётся через `OPENROUTER_MODEL`. Файл `.env` не должен попадать в Git.
